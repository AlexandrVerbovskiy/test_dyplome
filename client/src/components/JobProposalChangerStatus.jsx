...
const JobProposalChangerStatus = ({ actualStatus, isSeller = false, isBuyer = false }) => {
    let nextStatus = null;
    ...
    if (isBuyer) {
        ...
        if (actualStatus == jobStatus["awaitingCancellationConfirmation"]["value"]) {
            nextStatus = "rejected";
        }
    }
    ...
    return <div>
        <button type="button" className={`btn btn-${nextStatusInfo["color"]} px-5`}>Make "{nextStatusInfo["text"]}"</button>
        {isSeller && <button type="button" className={`btn btn-${rejectStatusInfo["color"]} px-5`}>Make "{rejectStatusInfo["text"]}"</button>}
    </div>;
}
