...
class JobProposal extends Model {
  ...
  checkJobOwner = async (proposalId, userId) => await this.errorWrapper(async () => {
    const proposals = await this.dbQueryAsync(`SELECT job_requests.* FROM job_requests join jobs on jobs.id = job_requests.job_id WHERE job_requests.id = ? AND jobs.user_id = ?`, [proposalId, userId]);
    return proposals.length;
  });
}
