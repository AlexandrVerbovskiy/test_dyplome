...
class Action extends Model {
    ...
    getByKeyAndType = async (key, type, userId) => await this.errorWrapper(async () => {
        await this.dbQueryAsync("SELECT data FROM users_actions WHERE user_id = ? AND `key` = ? AND `type` = ?", [userId, key, type]);
    });

    getById = async (id, userId) => await this.errorWrapper(async () => {
        await this.dbQueryAsync("SELECT data FROM users_actions WHERE user_id = ? AND id = ?", [userId, id]);
    });
}
